...
if (isMainThread) {
  ...
  function startDaemon(opts = {}) {
    ...
    const sessionsDir = opts.sessionsDir ?? require("./persist.ts").SESSIONS_DIR;
    const vaultDir    = opts.vaultDir    ?? require("./vault.ts").getVaultDir();
...
// ─── WORKER THREAD ───────────────────────────────────────────────────────────
  ...
  async function extractKnowledge(sessionId, data) {
    ...
    const { extractWithOllama, flattenMessages } = require("./extract.ts");
  ...
  async function generateDigest() {
    ...
    try {
      ...
      const { ollamaRequest } = require("./extract.ts");
...
${sessionLines}`;
  ...
  async function mineWeaknesses() {
    ...
    miningRunning = true;
    try {
      const { listLogs, readLog } = require("./telemetry.ts");
