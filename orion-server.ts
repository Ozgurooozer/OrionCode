...
require("./core/credentials.ts").load();
require("./core/accounts.ts").applyActive(); // aktif hesap profili credentials üzerine biner
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/tasks") {
      const scheduler = (() => { try { return require("./core/scheduler.ts"); } catch { return null; } })();
      const queue     = (() => { try { return require("./core/queue.ts");     } catch { return null; } })();
